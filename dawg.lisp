...
(defun sxhash-sibling (node)
  (declare #.*fastest*)
  (if (null node)
      (sxhash nil)
    (with-slots (hash-sib) (the node node)
      (when (= -1 hash-sib)
        (setf hash-sib (logxor (sxhash (node-label node))
                               (sxhash-sibling (node-sibling node)))))
      hash-sib)))

(defun memoize (node ch-memo si-memo)
  (if (null node)
      nil
    (or (dict:get node ch-memo)
        (progn (setf (node-child node) (memoize (node-child node) ch-memo si-memo))
               (dict:get node ch-memo))
        (setf (dict:get node ch-memo) node))))
...
(defun insert (in parent ch-memo si-memo)
  (declare #.*fastest*)
  (let ((node (node-child parent)))
    (if (or (null node)
            (/= (byte-stream:peek in) (node-label node)))
        (progn
          (setf (node-child parent) (memoize node ch-memo si-memo))
          (push-child in parent))
      (insert (byte-stream:eat in) node ch-memo si-memo))))

(defun build-from-file (filepath)
  (declare #.*fastest*)
  (let* ((trie (make-node))
         (ch-memo (dict:make :test #'node= :hash #'sxhash-node))
         (si-memo (dict:make :test #'sibling= :hash #'sxhash-sibling))
         (cnt 0))
    (declare (fixnum cnt))
    (each-file-line-bytes (bytes beg end filepath)
      ...
      (let ((in (byte-stream:make bytes :start beg :end end)))
        ...
        (values (insert in trie ch-memo si-memo))))
    (setf trie (memoize trie ch-memo si-memo))
    trie))
